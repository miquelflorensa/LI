...
satVariable( done(T,H) ):- task(T), integer(H).    %"task T it's beeing done at hour H"
...
writeClauses(MaxHours):-
    ...
    writeDones(MaxHours),
    noOverlaping(MaxHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachTaskStartsOnce(MaxHours):- task(T), duration(T,D), MaxHourStart is MaxHours - D + 1,
    findall( start(T,H), between(1,MaxHourStart,H), Lits), exactly(1, Lits), fail.
eachTaskStartsOnce(_).

writeDones(MaxHours):- task(T), duration(T,D), MaxHourStart is MaxHours - D + 1, between(1, MaxHourStart,HoraInici),
    HoraFinal is HoraInici + D - 1, between(HoraInici, HoraFinal, Hora), writeClause([-start(T,HoraInici) , done(T,Hora)]), fail.
writeDones(_).

noOverlaping(MaxHours):- resourceUnits(R,U), between(1, MaxHours, H), findall( done(T,H), usesResource(T,R), Lits ), atMost(U, Lits), fail.
noOverlaping(_).
