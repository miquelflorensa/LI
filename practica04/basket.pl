...
writeClauses(MaxCost):-
    ...
    writeHomes,
    writeDoubles,
    noTeamHasDoubleOnCertainRounds,
    certainTeamsCantPlayAtHome,
    noTriples,
    maxCost(MaxCost),
    true,!.
...
eachTeamPlaysOnce:- difTeams(T,S),
    findall( match(T,S,R), round(R), LitsH ),
    findall( match(S,T,R), round(R), LitsA ),
    append(LitsH, LitsA, Lits), exactly(1,Lits), fail.
eachTeamPlaysOnce.

tvMatches:-
    round(R), findall( match(T,S,R), tvMatch(S,T), Lits ),
    atLeast(1, Lits), fail.
tvMatches.

writeHomes:- difTeams(T,S), round(R),
    writeClause([-match(T,S,R) , home(T,R)]),
    writeClause([-match(T,S,R) , -home(S,R)]), fail.
writeHomes.

writeDoubles:- team(T), round(R1), R2 is R1 + 1, round(R2),
    writeClause([-home(T,R1) , -home(T,R2), double(T,R2)]),
    writeClause([-home(T,R1) , home(T,R2), -double(T,R2)]),
    writeClause([home(T,R1) , -home(T,R2), -double(T,R2)]),
    writeClause([home(T,R1) , home(T,R2), double(T,R2)]), fail.
writeDoubles.


noTeamHasDoubleOnCertainRounds:- noDoubles(N), member(R,N),
    findall( double(T,R) , team(T), Lits ), exactly(0,Lits), fail.
noTeamHasDoubleOnCertainRounds.

certainTeamsCantPlayAtHome:- notHome(T, N),
    findall( home(T,R), member(R,N), Lits ), exactly(0, Lits), fail.
certainTeamsCantPlayAtHome.

noTriples:- team(T), round(R1), R1 > 1, R2 is R1 +1, round(R2),
    writeClause([-double(T,R1) , -double(T,R2)]), fail.
noTriples.
