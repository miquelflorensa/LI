...
incompatible(S1,S2):- student(S1), student(S2), S1 \= S2, lastYearTeam(T), member(S1,T), member(S2,T).

%%%%%%  SAT Variables:
satVariable( st(S,T) ):- student(S), team(T). % student S goes to team T

writeClauses:- 
	eachStudentExactlyOneTeam,
	eachTeamSize,
	atMostOneStudentIncompatible,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.



eachStudentExactlyOneTeam:- student(S), findall( st(S,T), team(T), Lits), exactly(1, Lits),fail.
eachStudentExactlyOneTeam.

eachTeamSize:- team(T), findall( st(S,T), student(S), Lits), teamSize(TS), exactly(TS, Lits),fail.
eachTeamSize.


atMostOneStudentIncompatible:- incompatible(S1,S2), incompatible(S1,S3), S2 \= S3, team(T), writeClause([-st(S1,T),-st(S2,T),-st(S3,T)]), fail.
atMostOneStudentIncompatible.
