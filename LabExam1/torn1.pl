...
writeClauses(K):-
    eachCExactly1E,         % each course has exactly one exam
    atMost5EperW,           % at most 5 exams per week
    eachTAtMost1EperW,      % each teacher has at most one exam per week
    eachSAtMost2EperW,      % each student has at most two exams per week
    defineLast,             % definition of last
    atMostKSLast(K).        % at most K last students

eachCExactly1E :-
    course(C),
    findall(courseWeek-C-W, week(W), Lits),
    exactly(1, Lits), fail.
eachCExactly1E.

atMost5EperW :-
    week(W),
    findall(courseWeek-C-W, course(C), Lits),
    atMost(5, Lits), fail.
atMost5EperW.

eachTAtMost1EperW :-
    teacher(T), week(W),
    findall(courseWeek-C-W, course(C, T), Lits),
    atMost(1, Lits), fail.
eachTAtMost1EperW.

eachSAtMost2EperW :-
    student(S), week(W),
    findall(courseWeek-C-W, studentCourses(S, C), Lits),
    atMost(2, Lits), fail.
eachSAtMost2EperW.

defineLast :-
    student(S), studentCourses(S, C),
    writeClause([\+courseWeek-C-3, last-S]), fail.
defineLast.

atMostKSLast(K) :-
    findall(last-S, student(S), Lits),
    atMost(K, Lits), fail.
atMostKSLast(_).
