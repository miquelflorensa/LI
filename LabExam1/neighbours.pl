...
incompatible(N1,N2):- 
	neighborNeighborhood(N1,NN1), 
	neighborNeighborhood(N2,NN2), NN1 == NN2.
...
writeClauses:-   
	nRepresentatives,
	eachRepresentativeDiffNeigh,
	eachNeiRepresentedByAtLeastOneRep,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

nRepresentatives:- findall( chosen(N) , neighbor(N), Lits), 
	numRepresentatives(Nmax), exactly(Nmax, Lits), fail. 
nRepresentatives.

eachRepresentativeDiffNeigh:- neighbor(N1), neighbor(N2),
incompatible(N1,N2), N1 \= N2, writeClause([-chosen(N1),-chosen(N2)]), fail.
eachRepresentativeDiffNeigh.
