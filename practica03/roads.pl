...
oneTwoStations:- road(C,S), writeClause([install(C),install(S)]),fail.
oneTwoStations.


maxStats:- findall( install(C), city(C), Lits), maxStations(N), atMost(N,Lits),fail.
maxStats.
%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

%displaySol(M):- nl, write(M), nl, nl, fail.
displaySol(M):- member(install(C),M), write("Gas station in city "), write(C),nl,fail.
displaySol(_).    
