...
incompatible(S1,S2):- expertise(S1,E1), expertise(S2,E2), E1 == E2.
...
exactlyStudents:- findall(chosen(S), student(S), Lits), numMembersTeam(N), exactly(N,Lits),fail.
exactlyStudents.

compatibleStudents:- student(S1), student(S2),
incompatible(S1,S2), S1 \= S2, writeClause([-chosen(S1),-chosen(S2)]), fail.
compatibleStudents.

studentsFriends:- student(S1),student(S2),not(friends(S1,S2)),writeClause([-chosen(S1),-chosen(S2)]),fail.
studentsFriends.
