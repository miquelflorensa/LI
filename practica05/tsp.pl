...
lowerBoundOfRemainingCities([City|Cities],Km):- lowerBoundOfRemainingCities(Cities,Km1),
    mindist(City,Km2), Km is Km1 + Km2, !.


myselect(CurrentCity, Cities, CityList):-
    findall((Km, C), (member(C,Cities), distance(CurrentCity, C,Km)), ListOfCities),
    sort(ListOfCities, SortedList),
    findall(C, member((_,C), SortedList), CityList),!.
...
tsp( Cities, AccumulatedKm, [ CurrentCity | RouteSoFar ] ):-
    ...
    myselect( CurrentCity, Cities, CityList ),  % implement this
    select(City, CityList, RemainingCities),
